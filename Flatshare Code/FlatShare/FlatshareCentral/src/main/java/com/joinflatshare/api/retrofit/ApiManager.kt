...
class ApiManager() {
    ...
    // User
    fun updateProfile(
        showProgress: Boolean, modelUser: User?, onResponseCallback: OnResponseCallback<Any?>?
    ) {
        if (ConnectivityListener.checkInternet()) {
            if (showProgress) {
                showProgress()
            ...
            CompositeDisposable().add(getClient()?.updateProfile(modelUser)!!
                .observeOn(AndroidSchedulers.mainThread()).subscribeOn(Schedulers.io())
                .subscribe({ trendsResponse ->
                    CommonMethods.registerUser(trendsResponse as UserResponse)
                    sendResponse(trendsResponse, onResponseCallback)
                }) { throwable -> handleError(throwable as Throwable, onResponseCallback) })
        }
    }
