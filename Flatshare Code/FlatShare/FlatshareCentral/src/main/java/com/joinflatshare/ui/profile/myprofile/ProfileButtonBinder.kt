...
class ProfileButtonBinder(
    ...
    fun setAdapter() {
        ...
        if (activity is ProfileActivity && binder is ActivityProfileBinding) {
            ...
            binder.rvExploreButtons.removeItemDecoration(itemDecoration!!)
            binder.rvExploreButtons.visibility = View.VISIBLE
            binder.rvExploreButtons.layoutManager =
                LinearLayoutManager(activity, LinearLayoutManager.HORIZONTAL, false)
            if (adapter == null) {
                binder.rvExploreButtons.viewTreeObserver.addOnGlobalLayoutListener(
                    object :
                        ViewTreeObserver.OnGlobalLayoutListener {
                        override fun onGlobalLayout() {
                            binder.rvExploreButtons.viewTreeObserver.removeOnGlobalLayoutListener(
                                this
                            )
                            adapter = ProfileAdapter(
                                activity,
                                buttons,
                                CommonMethods.getScreenWidth()
                            )
                            binder.rvExploreButtons.adapter = adapter
                        }
                    })
            } else adapter?.notifyDataSetChanged()
        }
