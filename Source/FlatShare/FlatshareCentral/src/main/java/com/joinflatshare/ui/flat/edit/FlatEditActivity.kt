...
class FlatEditActivity : BaseActivity() {
    ...
    override fun onRequestPermissionsResult(
        ...
        if (allPermitted) pickImage() else CommonMethods.makeToast( "Permission not provided")
    }

    fun pickImage() {
        ImageHelper.pickImage(this, 16f, 9f)
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (resultCode == AutocompleteActivity.RESULT_ERROR) {
                ...
                CommonMethods.makeToast( status.statusMessage)
                CommonMethod.makeLog("Error", status.statusMessage)
            } else if (resultCode == Activity.RESULT_CANCELED) {
                // The user canceled the operation.
            } else if (resultCode == ImagePicker.RESULT_ERROR) {
                CommonMethods.makeToast( getError(data))
            }
        }
    }
